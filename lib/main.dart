...
class AppState extends ChangeNotifier {
  ...
  void clearColdBoreEntries({required String sessionId}) {
    final idx = _sessions.indexWhere((s) => s.id == sessionId);
    if (idx < 0) return;
    final s = _sessions[idx];

    final keptShots = s.shots.where((shot) => !shot.isColdBore).toList();
    final keptShotsByString = <String, List<ShotEntry>>{
      for (final entry in s.shotsByString.entries)
        entry.key: entry.value.where((shot) => !shot.isColdBore).toList(),
    };

    _sessions[idx] = s.copyWith(
      shots: keptShots,
      shotsByString: keptShotsByString,
    );
    notifyListeners();
  }
  ...
  bool updateSessionShotCount({
    required String sessionId,
    required int shotCount,
    bool applyToRifle = false,
  }) {
    final idx = _sessions.indexWhere((s) => s.id == sessionId);
    if (idx < 0) return false;
    final session = _sessions[idx];

    var didApplyToRifle = false;
    if (applyToRifle && !session.shotCountAppliedToRifle) {
      final rifleId = session.rifleId;
      if (rifleId != null) {
        final rifleIdx = _rifles.indexWhere((r) => r.id == rifleId);
        if (rifleIdx >= 0) {
          final rifle = _rifles[rifleIdx];
          _rifles[rifleIdx] = rifle.copyWith(
            manualRoundCount: rifle.manualRoundCount + shotCount,
            barrelRoundCount: rifle.barrelRoundCount + shotCount,
          );
          didApplyToRifle = true;
        }
      }
    }
    ...
    _sessions[idx] = session.copyWith(
      ...
      shotCountAppliedToRifle:
          session.shotCountAppliedToRifle || didApplyToRifle,
    );
    notifyListeners();
    return didApplyToRifle;
  }
  ...
  final List<_TutorialStep> _steps = const [
    ...
    _TutorialStep(
      ...
      actions: [
        ...
        'Use Analyze target photo in Session Photos to confirm round count and mark a cold bore impact.',
...
class SessionDetailScreen extends StatelessWidget {
  ...
  Future<Uint8List?> _pickTargetPhotoBytes(BuildContext context) async {
  ...
  Future<String?> _resolveTargetAnalysisColdBoreConflict(
    BuildContext context,
  ) async {
    return showDialog<String>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Cold bore already exists'),
        content: const Text(
          'This session already has cold bore entries. Do you want to replace them, keep them and save round count only, or add another cold bore entry?',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop('countOnly'),
            child: const Text('Count only'),
          ),
          TextButton(
            onPressed: () => Navigator.of(context).pop('addAnother'),
  ...
  Future<void> _analyzeTargetPhoto(
    BuildContext context,
    TrainingSession s,
  ) async {
    if (!await _guardWrite(context)) return;

    final photoBytes = await _pickTargetPhotoBytes(context);
    if (photoBytes == null) return;
    if (!context.mounted) return;

    final result = await Navigator.of(context).push<_TargetAnalysisResult>(
      MaterialPageRoute(
        builder: (_) => _TargetPhotoAnalysisScreen(
          photoBytes: photoBytes,
          canApplyToRifle: s.rifleId != null,
          shotCountAlreadyApplied: s.shotCountAppliedToRifle,
        ),
      ),
    );
    if (result == null || !context.mounted) return;

    var createColdBore = result.createColdBoreEntry;
    if (createColdBore && s.shots.any((shot) => shot.isColdBore)) {
      final choice = await _resolveTargetAnalysisColdBoreConflict(context);
      if (choice == null) return;
      if (choice == 'countOnly') {
        createColdBore = false;
      } else if (choice == 'replace') {
        state.clearColdBoreEntries(sessionId: s.id);
      }
    }

    final didApplyToRifle = state.updateSessionShotCount(
      sessionId: s.id,
      shotCount: result.totalShotCount,
      applyToRifle: result.applyShotCountToRifle,
    );
    ...
    if (createColdBore) {
      state.addColdBoreEntry(
        sessionId: s.id,
        time: DateTime.now(),
        distance: result.coldBoreDistance.isEmpty
            ? '100 yd'
            : result.coldBoreDistance,
        result: result.coldBoreResult.isEmpty
            ? 'Impact OK'
            : result.coldBoreResult,
        notes: result.coldBoreNotes,
        offsetX: result.coldBoreOffsetX,
        offsetY: result.coldBoreOffsetY,
        offsetUnit: 'in',
      ...
      state.addSessionPhoto(
        ...
        caption: 'Analyzed target',
      );
    }

    if (!context.mounted) return;
    final parts = <String>['Saved ${result.totalShotCount} rounds'];
    if (didApplyToRifle) parts.add('applied to rifle');
    if (createColdBore) parts.add('cold bore created');
    ScaffoldMessenger.of(
      context,
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  OutlinedButton.icon(
                    onPressed: () => _analyzeTargetPhoto(context, s),
                    icon: const Icon(Icons.analytics_outlined),
                    label: const Text('Analyze target'),
                  ),
                  OutlinedButton.icon(
...
class _TargetImpactMarker {
  final String id;
  final Offset normalizedOffset;

  const _TargetImpactMarker({required this.id, required this.normalizedOffset});
}

class _TargetAnalysisResult {
  final Uint8List photoBytes;
  final int totalShotCount;
  final bool applyShotCountToRifle;
  final bool createColdBoreEntry;
  final String coldBoreDistance;
  final String coldBoreResult;
  final String coldBoreNotes;
  final double? coldBoreOffsetX;
  final double? coldBoreOffsetY;

  const _TargetAnalysisResult({
    required this.photoBytes,
    required this.totalShotCount,
    required this.applyShotCountToRifle,
    required this.createColdBoreEntry,
    required this.coldBoreDistance,
    required this.coldBoreResult,
    required this.coldBoreNotes,
    required this.coldBoreOffsetX,
    required this.coldBoreOffsetY,
  });
}

class _TargetPhotoAnalysisScreen extends StatefulWidget {
  final Uint8List photoBytes;
  final bool canApplyToRifle;
  final bool shotCountAlreadyApplied;

  const _TargetPhotoAnalysisScreen({
    required this.photoBytes,
    required this.canApplyToRifle,
    required this.shotCountAlreadyApplied,
  });

  @override
  State<_TargetPhotoAnalysisScreen> createState() =>
      _TargetPhotoAnalysisScreenState();
}

class _TargetPhotoAnalysisScreenState extends State<_TargetPhotoAnalysisScreen> {
  final _shotCountCtrl = TextEditingController(text: '0');
  final _distanceCtrl = TextEditingController(text: '100 yd');
  final _resultCtrl = TextEditingController(text: 'Impact OK');
  final _notesCtrl = TextEditingController(text: 'Marked from target analysis.');

  final List<_TargetImpactMarker> _impacts = <_TargetImpactMarker>[];

  Offset? _centerPoint;
  Offset? _scalePoint;
  Size? _imagePixelSize;
  int? _coldBoreImpactIndex;
  bool _markColdBore = false;
  bool _applyShotCountToRifle = false;
  bool _shotCountDirty = false;
  bool _updatingShotCountText = false;
  String _tapMode = 'impact';

  @override
  void initState() {
    super.initState();
    _shotCountCtrl.addListener(() {
      if (_updatingShotCountText) return;
      _shotCountDirty = true;
    });
  ...
  void _syncShotCountIfNeeded() {
    if (_shotCountDirty) return;
    _updatingShotCountText = true;
    _shotCountCtrl.text = _impacts.length.toString();
    _updatingShotCountText = false;
  }
  ...
  void _handleImageTap(TapUpDetails details, BoxConstraints constraints) {
    ...
    setState(() {
      switch (_tapMode) {
        case 'center':
          _centerPoint = normalized;
          break;
        case 'scale':
          _scalePoint = normalized;
          break;
        case 'impact':
        default:
          _impacts.add(
            _TargetImpactMarker(
              id: '${DateTime.now().microsecondsSinceEpoch}_${_impacts.length}',
              normalizedOffset: normalized,
            ),
          );
          _syncShotCountIfNeeded();
          break;
      }
    });
  }
  ...
  double? _pixelsPerInch() {
    ...
    final scale = _scalePoint;
    final imageSize = _imagePixelSize;
    if (center == null || scale == null || imageSize == null) return null;
    final dx = (scale.dx - center.dx) * imageSize.width;
    final dy = (scale.dy - center.dy) * imageSize.height;
    final dist = math.sqrt((dx * dx) + (dy * dy));
    return dist > 6 ? dist : null;
  }

  (double, double)? _impactOffsetInches(_TargetImpactMarker marker) {
    final center = _centerPoint;
    final imageSize = _imagePixelSize;
    final pixelsPerInch = _pixelsPerInch();
    if (center == null || imageSize == null || pixelsPerInch == null) {
      return null;
    }

    final dxPixels = (marker.normalizedOffset.dx - center.dx) * imageSize.width;
    final dyPixels = (center.dy - marker.normalizedOffset.dy) * imageSize.height;
    return (dxPixels / pixelsPerInch, dyPixels / pixelsPerInch);
  }

  void _removeImpact(int index) {
    setState(() {
      _impacts.removeAt(index);
      if (_coldBoreImpactIndex == index) {
        _coldBoreImpactIndex = null;
      } else if (_coldBoreImpactIndex != null && _coldBoreImpactIndex! > index) {
        _coldBoreImpactIndex = _coldBoreImpactIndex! - 1;
      }
      _syncShotCountIfNeeded();
    });
  }
  ...
  void _save() {
    final totalShotCount = int.tryParse(_shotCountCtrl.text.trim());
    if (totalShotCount == null || totalShotCount <= 0) return;

    double? coldBoreX;
    double? coldBoreY;
    if (_markColdBore) {
      if (_coldBoreImpactIndex == null ||
          _coldBoreImpactIndex! < 0 ||
          _coldBoreImpactIndex! >= _impacts.length) {
        return;
      }
      final offsets = _impactOffsetInches(_impacts[_coldBoreImpactIndex!]);
    ...
    Navigator.of(context).pop(
      _TargetAnalysisResult(
        photoBytes: widget.photoBytes,
        totalShotCount: totalShotCount,
        applyShotCountToRifle: _applyShotCountToRifle,
        createColdBoreEntry: _markColdBore,
        coldBoreDistance: _distanceCtrl.text.trim(),
        coldBoreResult: _resultCtrl.text.trim(),
        coldBoreNotes: _notesCtrl.text.trim(),
        coldBoreOffsetX: coldBoreX,
        coldBoreOffsetY: coldBoreY,
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    final pixelsPerInch = _pixelsPerInch();
    return Scaffold(
      appBar: AppBar(title: const Text('Analyze target photo')),
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          Text(
            'Tap center, then tap one grid spacing (1 inch), then add impacts. You can mark one impact as the cold bore shot before saving.',
            style: Theme.of(context).textTheme.bodyMedium,
          ),
          const SizedBox(height: 12),
          SegmentedButton<String>(
            segments: const [
              ButtonSegment(value: 'center', icon: Icon(Icons.gps_fixed), label: Text('Center')),
              ButtonSegment(value: 'scale', icon: Icon(Icons.straighten), label: Text('1 in')),
              ButtonSegment(value: 'impact', icon: Icon(Icons.adjust), label: Text('Impacts')),
            ],
            selected: <String>{_tapMode},
            onSelectionChanged: (value) => setState(() => _tapMode = value.first),
          ),
          const SizedBox(height: 12),
          Card(
            child: Padding(
              padding: const EdgeInsets.all(12),
              child: AspectRatio(
                aspectRatio: _imagePixelSize == null
                    ? 1
                    : (_imagePixelSize!.width / _imagePixelSize!.height),
                child: LayoutBuilder(
                  builder: (context, constraints) => GestureDetector(
                    onTapUp: (details) => _handleImageTap(details, constraints),
                    child: Stack(
                      fit: StackFit.expand,
                      children: [
                        ClipRRect(
                        ...
                        if (_centerPoint != null)
                          Positioned(
                            left: (_centerPoint!.dx * constraints.maxWidth) - 12,
                            top: (_centerPoint!.dy * constraints.maxHeight) - 12,
                            child: const Icon(Icons.add, size: 24, color: Colors.lightGreenAccent),
                          ),
                        if (_scalePoint != null)
                          Positioned(
                            left: (_scalePoint!.dx * constraints.maxWidth) - 12,
                            top: (_scalePoint!.dy * constraints.maxHeight) - 12,
                            child: const Icon(Icons.straighten, size: 24, color: Colors.amberAccent),
                          ),
                        for (var i = 0; i < _impacts.length; i++)
                          Positioned(
                            left: (_impacts[i].normalizedOffset.dx * constraints.maxWidth) - 14,
                            top: (_impacts[i].normalizedOffset.dy * constraints.maxHeight) - 14,
                            child: Container(
                              width: 28,
                              height: 28,
                              decoration: BoxDecoration(
                                color: _coldBoreImpactIndex == i
                                    ? Colors.redAccent
                                    : Theme.of(context).colorScheme.primary,
                                shape: BoxShape.circle,
                                border: Border.all(color: Colors.white, width: 1.5),
          ...
          Text(
            pixelsPerInch == null
                ? 'Calibration needed: set center and 1-inch reference.'
                : 'Calibration ready. Impacts can be converted to inches.',
          ),
          const SizedBox(height: 12),
          TextField(
            controller: _shotCountCtrl,
            keyboardType: TextInputType.number,
            decoration: const InputDecoration(
              labelText: 'Total round count',
              helperText: 'Defaults to the number of impacts you mark.',
            ),
          ),
          if (widget.canApplyToRifle)
            SwitchListTile(
              contentPadding: EdgeInsets.zero,
              value: _applyShotCountToRifle,
              onChanged: widget.shotCountAlreadyApplied
                  ? null
                  : (value) => setState(() => _applyShotCountToRifle = value),
              title: const Text('Apply round count to current rifle'),
              subtitle: Text(
                widget.shotCountAlreadyApplied
                    ? 'This session already applied round count to a rifle.'
                    : 'Adds the confirmed round count to the current session rifle.',
              ),
            ),
          SwitchListTile(
            contentPadding: EdgeInsets.zero,
            value: _markColdBore,
            onChanged: (value) => setState(() => _markColdBore = value),
            title: const Text('Create cold bore entry from one impact'),
            subtitle: const Text('Select one plotted impact as the cold bore shot.'),
          ),
          if (_markColdBore) ...[
            TextField(
              controller: _distanceCtrl,
              decoration: const InputDecoration(labelText: 'Cold bore distance'),
            ),
            TextField(
              controller: _resultCtrl,
              decoration: const InputDecoration(labelText: 'Cold bore result'),
            ),
            TextField(
              controller: _notesCtrl,
              decoration: const InputDecoration(labelText: 'Cold bore notes'),
              maxLines: 2,
            ),
          ],
          const SizedBox(height: 12),
          if (_impacts.isEmpty)
            const Text('No impacts marked yet.')
          else
            ..._impacts.asMap().entries.map((entry) {
              final offsets = _impactOffsetInches(entry.value);
              final offsetText = offsets == null
                  ? 'Set center + 1-inch reference to calculate offsets.'
                  : 'X ${offsets.$1.toStringAsFixed(2)} in • Y ${offsets.$2.toStringAsFixed(2)} in';
              return Card(
                child: ListTile(
                  title: Text('Impact ${entry.key + 1}'),
                  subtitle: Text(offsetText),
                  leading: Radio<int>(
                    value: entry.key,
                    groupValue: _coldBoreImpactIndex,
                    onChanged: _markColdBore
                        ? (value) => setState(() => _coldBoreImpactIndex = value)
                        : null,
                  ),
                  trailing: IconButton(
                    tooltip: 'Remove impact',
                    onPressed: () => _removeImpact(entry.key),
                    icon: const Icon(Icons.delete_outline),
                  ),
                ),
              );
