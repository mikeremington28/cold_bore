...
String _rifleDisplayLabel(
  ...
  if (includeName && name.isNotEmpty) parts.add(name);
  ...
  return parts.isEmpty ? 'Rifle' : _joinDisplayParts(parts);
}

String _riflePrimaryLabel(Rifle r) {
  final name = (r.name ?? '').trim();
  ...
  final caliber = r.caliber.trim();
  if (caliber.isNotEmpty) return caliber;
  return 'Rifle';
}

String _rifleSecondaryLabel(Rifle r) {
  final primary = _riflePrimaryLabel(r);
  final makeModel = _joinDisplayParts(
    [(r.manufacturer ?? '').trim(), (r.model ?? '').trim()],
    separator: ' ',
  );
  ...
  if (makeModel.isNotEmpty && makeModel != primary) parts.add(makeModel);
  if (caliber.isNotEmpty && caliber != primary) parts.add(caliber);
  if (serial.isNotEmpty) parts.add('SN $serial');

  final label = _joinDisplayParts(parts);
          ...
          child: ListView(
            children: [
              const ColdBoreSectionHeader(
                title: 'DOPE Library',
                subtitle:
                    'Quick reference, working chart, and ballistic tools.',
              ),
              const SizedBox(height: 12),
              ColdBoreCard(
                padding: const EdgeInsets.all(12),
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final currentRifle = recentSession?.rifleId != null
        ...
        final totalRounds = widget.state.rifles.fold<int>(
          0,
          (sum, rifle) => sum + rifle.manualRoundCount,
        );
        final verifiedDope = widget.state.ballisticDopeRecords
            .where((record) => record.isVerified)
            .length;
        final maintenanceDue = widget.state
        ...
        for (final shot in sessions.expand((s) => s.shots)) {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            currentRifle == null
                                ? 'No Rifle Assigned'
                                : _riflePrimaryLabel(currentRifle),
                            style: const TextStyle(
                              fontSize: 30,
                              fontWeight: FontWeight.w600,
                              height: 1,
                          ...
                          Text(
                            currentRifle == null
                                ? 'Select a rifle in Gear'
                                : _rifleSecondaryLabel(currentRifle),
                            style: TextStyle(
                              color: Colors.white.withValues(alpha: 0.72),
                            ),
                          ),
                          ...
                          SizedBox(
                            ...
                            child: FilledButton.tonal(
                              ...
                              child: const Text('View Rifle Details'),
                            ),
                          ),
                        ],
                      ),
                    ...
                    return ColdBoreCard(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 10),
                          Text(
                            'Rifle\n${currentRifle == null ? 'Unassigned' : _rifleDisplayLabel(currentRifle)}',
                          ),
                          const SizedBox(height: 6),
