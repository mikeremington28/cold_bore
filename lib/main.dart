...
const cbBg = Color(0xFF0F1115);
const cbSurface = Color(0xFF171A21);
const cbCard = Color(0xFF1E222B);
const cbCardAlt = Color(0xFF242A35);
const cbBorder = Color(0xFF343B49);
const cbBlue = Color(0xFF3B82F6);
const cbGreen = Color(0xFF22C55E);
const cbAmber = Color(0xFFF59E0B);
const cbRed = Color(0xFFEF4444);
const cbText = Color(0xFFF5F7FA);
const cbMuted = Color(0xFF9CA3AF);
const cbDim = Color(0xFF6B7280);
...
ThemeData _buildTacticalTheme() {
  ...
  return ThemeData(
    ...
    colorScheme: scheme,
    scaffoldBackgroundColor: cbBg,
    canvasColor: cbBg,
    cardColor: cbCard,
    dividerColor: cbBorder,
    appBarTheme: const AppBarTheme(
      backgroundColor: cbSurface,
    ...
    cardTheme: CardThemeData(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: const BorderSide(color: cbBorder),
      ),
    ...
    navigationBarTheme: NavigationBarThemeData(
      ...
      backgroundColor: cbSurface,
      surfaceTintColor: Colors.transparent,
      indicatorColor: cbBlue.withValues(alpha: 0.2),
      labelTextStyle: WidgetStateProperty.resolveWith(
        (states) => TextStyle(
          color: states.contains(WidgetState.selected) ? cbText : cbMuted,
      ...
      iconTheme: WidgetStateProperty.resolveWith(
        (states) => IconThemeData(
          color: states.contains(WidgetState.selected) ? cbBlue : cbMuted,
        ),
      ),
    ),
    inputDecorationTheme: InputDecorationTheme(
      filled: true,
      fillColor: cbCardAlt,
      helperStyle: const TextStyle(color: cbDim, fontSize: 12),
      border: OutlineInputBorder(
        borderRadius: BorderRadius.circular(16),
        borderSide: const BorderSide(color: cbBorder),
      ),
      ...
      focusedBorder: const OutlineInputBorder(
        ...
        borderSide: BorderSide(color: cbBlue, width: 1.8),
      ),
    ...
    textTheme: const TextTheme(
      ...
      bodyLarge: TextStyle(fontSize: 15, color: cbText),
      bodyMedium: TextStyle(fontSize: 14, color: cbMuted),
    ...
    chipTheme: ChipThemeData(
      backgroundColor: cbCardAlt,
      selectedColor: cbBlue.withValues(alpha: 0.2),
      secondarySelectedColor: cbBlue.withValues(alpha: 0.2),
      side: const BorderSide(color: cbBorder),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      labelStyle: const TextStyle(color: cbText),
      secondaryLabelStyle: const TextStyle(color: cbText),
...
class ColdBoreStatusPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final colors = switch (tone) {
      ColdBoreStatusTone.verified => const (cbGreen, Color(0xFFE9FFF2)),
      ColdBoreStatusTone.calculated => const (cbAmber, Color(0xFFFFF5DE)),
      ColdBoreStatusTone.warning => const (cbAmber, Color(0xFFFFF5DE)),
      ColdBoreStatusTone.danger => const (cbRed, Color(0xFFFFE7E7)),
      ColdBoreStatusTone.neutral => const (cbDim, cbText),
    };
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: cbBorder),
        color: cbCardAlt.withValues(alpha: 0.52),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        color: cbSurface,
        border: Border(
          top: BorderSide(color: cbBorder.withValues(alpha: 0.85)),
        ),
        boxShadow: [
          BoxShadow(
...
InputDecoration coldBoreDropdownStyle({required String label}) {
...
class AppThemeController extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    final raw = prefs.getString(kThemeModePrefsKey) ?? 'dark';
...
String _joinDisplayParts(
  Iterable<String> parts, {
  String separator = ' - ',
}) {
  final clean = parts.map((part) => part.trim()).where((part) => part.isNotEmpty);
  return clean.join(separator);
}

String _rifleDisplayLabel(
  Rifle r, {
  bool includeName = true,
  bool includeMakeModel = true,
  bool includeCaliber = true,
  bool includeSerial = false,
}) {
  final nickname = (r.name ?? '').trim();
  final manufacturer = (r.manufacturer ?? '').trim();
  final model = (r.model ?? '').trim();
  final makeModel = _joinDisplayParts([manufacturer, model], separator: ' ');
  final caliber = r.caliber.trim();
  final serial = (r.serialNumber ?? '').trim();

  final parts = <String>[];
  if (includeName && nickname.isNotEmpty) {
    parts.add(nickname);
    if (includeCaliber && caliber.isNotEmpty) parts.add(caliber);
  } else {
    if (includeMakeModel && makeModel.isNotEmpty) parts.add(makeModel);
    if (includeCaliber && caliber.isNotEmpty) parts.add(caliber);
    if (parts.isEmpty && includeName && nickname.isNotEmpty) parts.add(nickname);
  }
  if (includeSerial && serial.isNotEmpty) parts.add('SN $serial');

  return parts.isEmpty ? 'Rifle' : _joinDisplayParts(parts, separator: ' ');
}


String _riflePrimaryLabel(Rifle r) => _rifleDisplayLabel(r);

String _rifleSecondaryLabel(Rifle r) {
  final nickname = (r.name ?? '').trim();
  final makeModel = _joinDisplayParts(
  ...
  if (nickname.isNotEmpty && makeModel.isNotEmpty) parts.add(makeModel);
  if (serial.isNotEmpty) parts.add('SN $serial');

  final label = _joinDisplayParts(parts, separator: ' - ');
  return label.isEmpty ? 'Select a rifle in Gear' : label;
}


String _autoGeneratedIdentifierForUser(UserProfile user) {
  final compact = user.id.replaceAll(RegExp(r'[^A-Za-z0-9]'), '').toUpperCase();
  final fallback = DateTime.now().millisecondsSinceEpoch
      .toRadixString(36)
      .toUpperCase();
  final source = compact.isEmpty ? fallback : compact;
  final suffix = source.length >= 8
      ? source.substring(source.length - 8)
      : source.padLeft(8, '0');
  return 'CB_$suffix';
}
...
String _appleBackedIdentifierFromRecordName(String recordName) {
  final normalized = recordName.trim();
  if (normalized.isEmpty) return '';
  return 'APPLE_${_fnv1a64Hex(normalized)}';
}
...
class BallisticCalculationService {
  ...
  BallisticSolution calculate(BallisticSolverInput input) {
    ...
    final bcFactor =
        (1.0 / math.max(0.05, input.ballisticCoefficient)) * dragFactor;
    final densityScale =
...
class BallisticDopeRecord {
  ...
  double get verifiedElevation =>
      calculatedElevation + verifiedElevationCorrection;
  ...
  BallisticDopeRecord copyWith({
    ...
    return BallisticDopeRecord(
      ...
      verifiedWindCorrection:
          verifiedWindCorrection ?? this.verifiedWindCorrection,
...
Future<void> main() async {
  ...
  if (Firebase.apps.isNotEmpty) {
    ...
    debugPrint('Cloud sync initialization skipped: Firebase is unavailable.');
  }
...
class _AppRootState extends State<_AppRoot> with WidgetsBindingObserver {
  ...
  bool _autoICloudRestoreAttempted = false;
  ...
  Future<void> _loadState() async {
    try {
      ...
      await _ensureAppleBackedIdentifierIfAvailable();
      if (_firebaseReady) {
  ...
  Future<void> _ensureAppleBackedIdentifierIfAvailable() async {
    if (kIsWeb || defaultTargetPlatform != TargetPlatform.iOS) return;
    final activeUser = _state.activeUser;
    if (activeUser == null) return;

    try {
      final recordName = await _iCloudChannel.invokeMethod<String>(
        'getICloudUserRecordName',
      );
      if (recordName == null || recordName.trim().isEmpty) return;

      final appleIdentifier = _appleBackedIdentifierFromRecordName(recordName);
      if (appleIdentifier.isEmpty) return;

      final currentIdentifier = activeUser.identifier.trim().toUpperCase();
      if (currentIdentifier == appleIdentifier) return;

      _state.updateUserProfile(
        userId: activeUser.id,
        name: (activeUser.name ?? '').trim(),
        identifier: appleIdentifier,
      );
      _promptUniqueIdentifierOnHome = false;
      debugPrint(
        'Assigned Apple-backed identifier for active profile: $appleIdentifier',
      );
    } catch (e, st) {
      debugPrint('Apple-backed identifier unavailable: $e\n$st');
    }
  }
  ...
  bool _hasMeaningfulLocalData() {
    if (_state.rifles.isNotEmpty || _state.ammoLots.isNotEmpty) return true;
    if (_state.users.length > 1) return true;
    if (_state.allSessions.isEmpty) return false;

    for (final session in _state.allSessions) {
      final hasUserData =
          session.notes.trim().isNotEmpty ||
          session.locationName.trim().isNotEmpty ||
          session.rifleId != null ||
  ...
  Future<void> _runICloudBackupNow() async {
    ...
    // Safety: on fresh reinstall/empty state, do not overwrite cloud backup.
    if (_looksLikeFreshLocalInstall() && !_hasMeaningfulLocalData()) {
      debugPrint(
        '[iCloudBackup] Skipped auto backup to avoid overwriting cloud data from fresh local install state.',
      );
      return;
    }

    // If auto-restore has not had a chance to run on this launch, defer backup.
    if (!_autoICloudRestoreAttempted) {
      debugPrint('[iCloudBackup] Deferred until auto-restore attempt completes.');
      return;
    }
...
BallisticDopeRecord _ballisticDopeRecordFromMap(
...
class _HomeShellState extends State<HomeShell> {
  ...
  Future<void> _promptForUniqueIdentifierIfNeeded() async {
    ...
    final autoIdentifier = _autoGeneratedIdentifierForUser(activeUser);
...
class _DataScreenState extends State<DataScreen> {
  static const String _selectedRiflePrefsKey =
      'cold_bore.data_working_dope.selected_rifle.v1';
  static const String _selectedAmmoPrefsKey =
      'cold_bore.data_working_dope.selected_ammo.v1';

  bool _allDistances = true;
  bool _showBallisticHistory = false;
  ...
  Future<void> _loadPrefs() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        _selectedRifleFilterId = prefs.getString(_selectedRiflePrefsKey);
        _selectedAmmoFilterId = prefs.getString(_selectedAmmoPrefsKey);
      });
    } catch (e, st) {
      debugPrint('Data DOPE prefs load failed: $e\n$st');
    }
  }
  ...
  Future<void> _savePrefs() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final rifleId = _selectedRifleFilterId;
      final ammoId = _selectedAmmoFilterId;
      if (rifleId == null || rifleId.trim().isEmpty) {
        await prefs.remove(_selectedRiflePrefsKey);
      } else {
        await prefs.setString(_selectedRiflePrefsKey, rifleId);
      }
      if (ammoId == null || ammoId.trim().isEmpty) {
        await prefs.remove(_selectedAmmoPrefsKey);
      } else {
        await prefs.setString(_selectedAmmoPrefsKey, ammoId);
      }
    } catch (e, st) {
      debugPrint('Data DOPE prefs save failed: $e\n$st');
    }
  }
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final ammoLots = widget.state.ammoLots;
        final wmap = widget.state.workingDopeRifleAmmo;
        ...
        Future<void> deleteBallisticHistoryRow(
          BallisticDopeRecord record,
        ) async {
          if (!await _guardWrite(
            context,
            operation: 'Delete ballistic DOPE history row',
          )) {
            return;
          }
          final ok = await showDialog<bool>(
            ...
            builder: (_) => AlertDialog(
              ...
              content: const Text(
                'This removes the saved estimate/validation row from Ballistic DOPE history. Working DOPE entries are not changed.',
              ),
              actions: [
                TextButton(
        ...
        final recentRifleId = widget.state.sessions
            .map((s) => s.rifleId)
            .whereType<String>()
            .firstWhere((id) => id.trim().isNotEmpty, orElse: () => '');
        final validRifleIds = rifles.map((r) => r.id).toSet();
        final effectiveRifleId =
            (_selectedRifleFilterId != null &&
                    validRifleIds.contains(_selectedRifleFilterId))
                ? _selectedRifleFilterId
                : (recentRifleId.isNotEmpty
                    ? recentRifleId
                    : (rifles.isNotEmpty ? rifles.first.id : null));
        final selectedRifle = effectiveRifleId == null
            ? null
            : widget.state.rifleById(effectiveRifleId);
        final filteredAmmo = selectedRifle == null
            ? <AmmoLot>[]
            : ammoLots
                .where((a) => a.caliber.trim() == selectedRifle.caliber.trim())
                .toList();
        filteredAmmo.sort(
          (a, b) => ammoTitle(a.id)
              .toLowerCase()
              .compareTo(ammoTitle(b.id).toLowerCase()),
        );
        final validAmmoIds = filteredAmmo.map((a) => a.id).toSet();
        final effectiveAmmoId =
            (_selectedAmmoFilterId != null &&
                    validAmmoIds.contains(_selectedAmmoFilterId))
                ? _selectedAmmoFilterId
                : (filteredAmmo.isNotEmpty ? filteredAmmo.first.id : null);

        final selectedBucketKey =
            effectiveRifleId != null && effectiveAmmoId != null
                ? '${effectiveRifleId}_$effectiveAmmoId'
                : null;
        final selectedBucket = selectedBucketKey == null
            ? null
            : wmap[selectedBucketKey];

        var dks = selectedBucket == null
            ? <DistanceKey>[]
            : selectedBucket.keys.toList();
        dks.sort((a, b) => a.value.compareTo(b.value));
        if (!_allDistances) {
          dks = dks.where((dk) => dk.value.round() % 25 == 0).toList();
        }

        Widget workingDopeChart() {
          return ColdBoreCard(
            padding: const EdgeInsets.all(16),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const ColdBoreSectionHeader(
                  title: 'Working DOPE',
                  subtitle: 'Field reference for one firearm and ammo combination.',
                ),
                const SizedBox(height: 10),
                DropdownButtonFormField<String?>(
                  initialValue: effectiveRifleId,
                  isExpanded: true,
                ...
                DropdownButtonFormField<String?>(
                  ...
                  items: filteredAmmo
                      ...
                      .toList(),
                  onChanged: filteredAmmo.isEmpty
                      ? null
                      : (value) {
                ...
                const SizedBox(height: 10),
                if (effectiveRifleId == null)
                  Text(
                    'Add a firearm before Working DOPE can be displayed.',
                    style: TextStyle(
                ...
                else if (effectiveAmmoId == null)
                  Text(
                    'Add a matching ammo lot for ${rifleTitle(effectiveRifleId)}.',
                    style: TextStyle(
                      color: Theme.of(context)
                          .colorScheme
                          .onSurface
                  ...
                  Text(
                    'No Working DOPE saved for ${rifleTitle(effectiveRifleId)} / ${ammoTitle(effectiveAmmoId)}.',
                    style: TextStyle(
                      color: Theme.of(context)
                          .colorScheme
                          .onSurface
                  ...
                  ...dks.map((dk) {
                    ...
                    Future<void> editEntry() async {
                      ...
                      if (edited == null || selectedBucketKey == null) return;
                    ...
                    Future<void> deleteEntry() async {
                      ...
                      if (ok != true || selectedBucketKey == null) return;
                      final removed = widget.state.deleteWorkingDopeEntry(
                        rifleOnly: false,
                        bucketKey: selectedBucketKey,
                        distanceKey: dk,
                      );
                        ...
                        child: Row(
                          ...
                          children: [
                            Expanded(
                              flex: 3,
                              child: Text(
                                '${dk.value.toStringAsFixed(0)} ${distanceUnitShort(dk.unit)}',
                                style: const TextStyle(
                                  fontWeight: FontWeight.w700,
                                ),
                              ),
        ...
        Widget ballisticHistorySection() {
          return ColdBoreCard(
            padding: const EdgeInsets.all(0),
            child: Theme(
              data: Theme.of(context).copyWith(dividerColor: Colors.transparent),
              child: ExpansionTile(
                initiallyExpanded: _showBallisticHistory,
                onExpansionChanged: (value) =>
                    setState(() => _showBallisticHistory = value),
                title: const Text('Saved Ballistic DOPE History'),
                subtitle: Text(
                  ballisticRecords.isEmpty
                      ? 'No ballistic history rows yet.'
                      : '${ballisticRecords.length} saved row${ballisticRecords.length == 1 ? '' : 's'}',
                ),
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Wrap(
                                    ...
                                    children: [
                                      ColdBoreStatusPill(
                                        label: 'Elev ${elevationLabel(record)}',
                                      ),
                                      ColdBoreStatusPill(
                                        label: 'Wind ${windageLabel(record)}',
                                      ),
                                    ],
                                  ),
                                ],
        ...
        return Padding(
          ...
          child: ListView(
            children: [
              const ColdBoreSectionHeader(
                title: 'DOPE Library',
                subtitle: 'Working DOPE, saved charts, and ballistic history.',
              ),
              const SizedBox(height: 12),
              workingDopeChart(),
              const SizedBox(height: 12),
                ...
                builder: (context) {
                  ...
                  return ColdBoreCard(
                    child: ListTile(
                      ...
                      subtitle: Text(
                        snapshots.isEmpty
                            ? 'Add firearms to start tracking cleaning, torque, zero, and barrel reminders.'
                            : '$overdue overdue - $dueSoon due soon across ${snapshots.length} firearm${snapshots.length == 1 ? '' : 's'}',
                      ),
                      trailing: const Icon(Icons.chevron_right),
                      onTap: () {
                        Navigator.of(context).push(
...
class BallisticAssistantScreen extends StatefulWidget {
  ...
  State<BallisticAssistantScreen> createState() =>
      _BallisticAssistantScreenState();
}
...
class _BallisticAssistantScreenState extends State<BallisticAssistantScreen> {
  ...
  final TextEditingController _chartStartDistanceCtrl = TextEditingController(
    text: '100',
  );
  final TextEditingController _chartEndDistanceCtrl = TextEditingController(
    text: '1000',
  );
  final TextEditingController _chartIncrementCtrl = TextEditingController(
    text: '100',
  );
  ...
  final TextEditingController _zeroDistanceCtrl = TextEditingController(
    text: '100',
  );
  final TextEditingController _sightHeightCtrl = TextEditingController(
    text: '1.8',
  );
  final TextEditingController _muzzleVelocityCtrl = TextEditingController(
    text: '2650',
  );
  final TextEditingController _bcCtrl = TextEditingController(text: '0.45');
  final TextEditingController _elevationCorrectionCtrl = TextEditingController(
    text: '0',
  );
  final TextEditingController _windCorrectionCtrl = TextEditingController(
    text: '0',
  );
  ...
  Future<void> _calculateDope() async {
    ...
    if (distance == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Enter a valid distance.')));
      return;
    }

    final row = _buildCalculatedRow(
      distance: distance,
  ...
  Future<void> _saveChartToHistory() async {
    ...
    if (_chartRows.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Generate a chart first.')));
      return;
    }
  ...
  DopeEntry _dopeEntryFromChartRow(
    _GeneratedDopeRow row,
    BallisticDopeRecord? record,
  ) {
  ...
  Future<void> _saveSelectedRowToWorkingDope() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Selected chart row saved to Working DOPE.'),
      ),
    );
  }
  ...
  Future<void> _validateChartRow(int index) async {
    ...
    final updated = widget.state.validateBallisticDope(
      ...
      outcome: outcome,
      elevationCorrection:
          outcome == BallisticValidationOutcome.adjustmentRequired
          ? elevationCorrection
          : 0.0,
    ...
    final confirmations = widget.state.verifiedBallisticConfirmationCount(
      updated,
    );
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
  ...
  Future<void> _saveValidation() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'Verified DOPE saved. $confirmations confirmation${confirmations == 1 ? '' : 's'}.',
        ),
      ),
    );
  }
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        if (_selectedAmmoId != null &&
            !ammoOptions.any((a) => a.id == _selectedAmmoId)) {
          _selectedAmmoId = null;
        }
        ...
        final activeConfirmations = active == null
            ? 0
            : widget.state.verifiedBallisticConfirmationCount(active);
        final recordsById = {for (final r in records) r.id: r};

        final Color unverifiedCardBg =
            Theme.of(context).brightness == Brightness.dark
            ? const Color(0xFF4A3A14)
            : const Color(0xFFFFF1C2);
        final Color verifiedCardBg =
            Theme.of(context).brightness == Brightness.dark
              ...
              ColdBoreCard(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Ballistic solutions are estimates. Confirm at the range before relying on them.',
                        style: TextStyle(
                          color: Theme.of(context)
                              .colorScheme
                              .onSurface
                    ...
                    children: [
                      ...
                      if (records.isEmpty)
                        ...
                        ...visibleBallisticRecords.map((record) {
                          ...
                          final recAmmo = widget.state.ammoById(
                            record.ammoLotId,
                          );
                          final confirmations = widget.state
                              .verifiedBallisticConfirmationCount(record);
                          return Container(
                            margin: const EdgeInsets.only(bottom: 6),
                            padding: const EdgeInsets.all(10),
                            decoration: BoxDecoration(
...
class _GeneratedDopeRow {
  ...
  _GeneratedDopeRow copyWith({String? recordId}) {
    return _GeneratedDopeRow(
      distance: distance,
      distanceUnit: distanceUnit,
      outputUnit: outputUnit,
      ...
      body: ListView(
        ...
        children: [
          const ColdBoreSectionHeader(
            title: 'Profiles',
            subtitle:
                'Switch active user profiles used for session ownership and sharing.',
          ),
          const SizedBox(height: 10),
          ColdBoreCard(
            padding: const EdgeInsets.all(12),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 4),
                              Text(_displayUserIdentifier(u.identifier)),
                            ],
                          ),
                        ),
                        if (isActive)
                      ...
                      children: [
                        Expanded(
                          child: FilledButton.tonalIcon(
                            ...
                            label: Text(
                              isActive ? 'Current Profile' : 'Set Active',
                            ),
                          ),
                        ),
                        if (canDelete) ...[
...
class _SessionsScreenState extends State<SessionsScreen> {
  ...
  static const String _homeStatsRiflePrefsKey =
      'cold_bore.home_stats.selected_rifle.v1';
  ...
  Future<void> _loadUiPrefs() async {
    try {
      ...
      final savedHomeStatsRifleId = prefs.getString(_homeStatsRiflePrefsKey);
      if (!mounted) return;
      setState(() {
        if (saved != null) _filtersPanelCollapsed = saved;
        if (savedHomeStatsRifleId != null && savedHomeStatsRifleId.trim().isNotEmpty) {
          _homeStatsRifleId = savedHomeStatsRifleId;
  ...
  Future<void> _saveHomeStatsRiflePreference() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final rifleId = _homeStatsRifleId;
      if (rifleId == null || rifleId.trim().isEmpty) {
        await prefs.remove(_homeStatsRiflePrefsKey);
      } else {
        await prefs.setString(_homeStatsRiflePrefsKey, rifleId);
      }
    } catch (e, st) {
      debugPrint('Home stats rifle prefs save failed: $e\n$st');
    }
  }
  ...
  void _toggleSessionSelection(String sessionId) {
    setState(() {
      _sessionSelectionMode = true;
      if (_selectedSessionIds.contains(sessionId)) {
        _selectedSessionIds.remove(sessionId);
        if (_selectedSessionIds.isEmpty) _sessionSelectionMode = false;
      } else {
  ...
  List<TrainingSession> _selectedSessionsFrom(List<TrainingSession> sessions) =>
      sessions
  ...
  Widget _sessionTile(
    ...
    final weatherLine = weatherBits.isEmpty
        ? 'No weather saved'
        : weatherBits.join('  -  ');
    ...
    return ColdBoreCard(
      ...
      child: InkWell(
        ...
        onLongPress: () => _toggleSessionSelection(s.id),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Row(
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
              ...
              children: [
                Row(
                  children: [
                    ...
                    if (_sessionSelectionMode) ...[
                      TextButton(
                        onPressed: _clearSessionSelection,
                        child: const Text('Cancel'),
                      ),
                      const SizedBox(width: 6),
                      FilledButton.tonalIcon(
                        onPressed: _selectedSessionIds.isEmpty
                            ? null
                            : () async {
                                final selected = _selectedSessionsFrom(filtered);
                                await _bulkMoveToFolder(context, selected);
                                if (mounted) _clearSessionSelection();
                              },
                ...
                if (filtered.isEmpty)
                  ...
                  ...filtered.map((s) => _sessionTile(context, s, selectionMode: _sessionSelectionMode))
                else
                  ...groupKeys.expand((key) {
                    ...
                    return <Widget>[
                      ...
                      if (!collapsed)
                        ...items.map((s) => _sessionTile(context, s, selectionMode: _sessionSelectionMode)),
                    ];
                  }),
              ],
            ),
        ...
        final greeting = nowHour < 12
            ? 'Good Morning'
            : (nowHour < 18 ? 'Good Afternoon' : 'Good Evening');
        final displayName = (user.name ?? '').trim().isEmpty
            ? 'Shooter'
            : user.name!.trim();

        final recentSession = sessions.isEmpty ? null : sessions.first;
        final recentRifle = recentSession?.rifleId != null
            ? widget.state.rifleById(recentSession!.rifleId)
            : (widget.state.rifles.isEmpty ? null : widget.state.rifles.first);
        final currentAmmo = recentSession?.ammoLotId != null
            ? widget.state.ammoById(recentSession!.ammoLotId)
            : null;
        final validHomeRifleIds = widget.state.rifles.map((r) => r.id).toSet();
        final statsRifleId =
            (_homeStatsRifleManuallySelected &&
                    _homeStatsRifleId != null &&
                    validHomeRifleIds.contains(_homeStatsRifleId))
                ? _homeStatsRifleId
                : (recentRifle?.id ??
                    (widget.state.rifles.isNotEmpty
                        ? widget.state.rifles.first.id
                        : null));
        final currentRifle = statsRifleId == null
            ? null
            : widget.state.rifleById(statsRifleId);

        final tempF =
            widget.state.temperatureF ?? recentSession?.temperatureF ?? 68.0;
        final windMph =
            widget.state.windSpeedMph ?? recentSession?.windSpeedMph ?? 5.0;
        final windDeg =
        ...
        final totalRounds = currentRifle?.manualRoundCount ?? 0;
        final verifiedDope = statsRifleId == null
            ? 0
            : widget.state.ballisticDopeRecords
                .where((record) => record.isVerified && record.rifleId == statsRifleId)
                .length;
        final selectedMaintenance = statsRifleId == null
            ? null
            : (() {
                try {
                  return widget.state.maintenanceSnapshotForRifle(statsRifleId);
                } catch (_) {
                  return null;
                }
              })();
        final maintenanceDue = selectedMaintenance == null
            ? 0
            : (selectedMaintenance.overallStatus == MaintenanceDueStatus.good
                ? 0
                : 1);

        final coldBoreMoa = <double>[];
        for (final session in sessions) {
          if (statsRifleId != null && session.rifleId != statsRifleId) continue;
          for (final shot in session.shots) {
            if (!shot.isColdBore) continue;
            if (shot.offsetX == null || shot.offsetY == null) continue;
            final mx = _shotOffsetToMoa(shot, shot.offsetX!);
            final my = _shotOffsetToMoa(shot, shot.offsetY!);
            coldBoreMoa.add(math.sqrt((mx * mx) + (my * my)));
          }
        }
        final coldBoreAvg = coldBoreMoa.isEmpty
            ? null
            : (coldBoreMoa.reduce((a, b) => a + b) / coldBoreMoa.length);

        String commaInt(int value) {
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: const LinearGradient(
                    ...
                    colors: [Color(0xFF0B1730), Color(0xFF070E1E)],
                  ),
                ),
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Icon(icon, color: const Color(0xFF2F8BFF), size: 22),
                    const SizedBox(height: 8),
                    Text(
                      titleA,
                      textAlign: TextAlign.center,
        ...
        return Container(
          decoration: const BoxDecoration(
            gradient: RadialGradient(
              center: Alignment(-0.1, -1.1),
              radius: 1.2,
              colors: [Color(0xFF0C2A55), Color(0xFF050914), Color(0xFF02050C)],
              stops: [0.0, 0.58, 1.0],
            ),
          ),
          child: ListView(
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      gradient: const LinearGradient(
                        colors: [Color(0xFF121E33), Color(0xFF0A1325)],
                      ),
                    ),
                    child: Row(
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Last sync: ${fmtTime(DateTime.now())}',
                              style: TextStyle(
                                fontSize: 11.5,
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: const LinearGradient(
                    ...
                    colors: [Color(0xFF152640), Color(0xFF0A1526)],
                  ),
                ),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                'Feels like ${(tempF - 2).toStringAsFixed(0)}\u00B0F',
                                style: TextStyle(
                  ...
                  return Wrap(
                    ...
                    children: [
                      ...
                      quickAction(
                        ...
                        titleB: 'Rifles',
                        onTap: () => openPage(
                          EquipmentScreen(state: widget.state, showAppBar: true),
                        ),
                      ),
                      quickAction(
                        width: tileWidth,
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              Expanded(
                                child: Text(
                                  'Round Count\n${commaInt(currentRifle?.manualRoundCount ?? 0)}',
                                ),
                              ),
                              Expanded(
                                child: Text(
                                  'Cold Bore Avg\n${coldBoreAvg == null ? 'N/A' : '${coldBoreAvg.toStringAsFixed(1)} MOA'}',
                                ),
                              ),
                              Expanded(
                                child: Text(
                                  'Verified DOPE\n$verifiedDope Entries',
                                ),
                              ),
                            ],
                          ),
                          ...
                          SizedBox(
                            ...
                            child: FilledButton.tonal(
                              ...
                              child: const Text('View Firearm Details'),
                            ),
                          ),
                        ],
                      ),
                    ...
                    return ColdBoreCard(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 10),
                          Text(
                            'Firearm\n${recentRifle == null ? 'Unassigned' : _rifleDisplayLabel(recentRifle)}',
                          ),
                          const SizedBox(height: 6),
                          Text('Ammo\n${currentAmmo?.name ?? 'Not set'}'),
                          const SizedBox(height: 6),
                          Text(
                            'Location\n${recentSession?.locationName.trim().isEmpty ?? true ? 'Kestrel Range' : recentSession!.locationName.trim()}',
                          ),
                          const SizedBox(height: 8),
                          Row(
                            children: [
                  ...
                  return Wrap(
                    ...
                    children: [
                      metricCard(
                        ...
                        subtitle: currentRifle == null ? 'No firearm selected' : _rifleDisplayLabel(currentRifle),
                      ),
                      metricCard(
                        title: 'COLD BORE AVG',
                        ...
                        subtitle: currentRifle == null ? 'No firearm selected' : 'Selected firearm average',
                      ),
                      metricCard(
                        title: 'VERIFIED DOPE',
                        value: '$verifiedDope',
                        subtitle: currentRifle == null ? 'No firearm selected' : 'Selected firearm entries',
                      ),
                      metricCard(
                        title: 'SERVICE STATUS',
                        value: maintenanceDue == 0 ? 'Good' : 'Due',
                        subtitle: selectedMaintenance == null
                            ? 'No firearm selected'
                            : _maintenanceDueLabel(selectedMaintenance.overallStatus),
                      ),
                    ],
                  );
                },
      ...
      body: ListView(
        ...
        children: [
          const ColdBoreSectionHeader(
            title: 'Timer Tool',
            subtitle:
                'Run standalone timing drills with shot split capture and optional audio assist.',
          ),
          const SizedBox(height: 8),
...
class SessionDetailScreen extends StatelessWidget {
  ...
  String sessionRifleLabel(String rifleId) {
    ...
    if (rifle == null) return 'Deleted firearm ($rifleId)';
    return _rifleDisplayLabel(rifle);
  }
  ...
  Future<void> shareSessionFile(BuildContext context, TrainingSession s) async {
    try {
            ...
            children: [
              ...
              Builder(
                builder: (context) {
                  ...
                  final activeString = s.strings.firstWhere(
                    (meta) => meta.id == s.activeStringId,
                    orElse: () => s.strings.isNotEmpty
                        ? s.strings.last
                        : SessionStringMeta(
                            id: s.activeStringId,
                  ...
                  final rifleKey = activeString.rifleId ?? s.rifleId;
                  final ammoId = activeString.ammoLotId ?? s.ammoLotId;

                  if (rifleKey == null) {
                    return _HintCard(
                      icon: Icons.info_outline,
                      title: 'Select a firearm',
                      message:
                          'Choose a firearm in the current string to view Working DOPE.',
                    );
                  }
                  if (ammoId == null) {
                    return _HintCard(
                      ...
                      title: 'Select ammo',
                      message:
                          'Choose ammo in the current string to view Working DOPE.',
                    );
                  }

                  final ammoKey = '${rifleKey}_$ammoId';
                  final useMap =
                      state.workingDopeRifleAmmo[ammoKey] ?? <DistanceKey, DopeEntry>{};
                  const scopeLabel = 'Firearm + Ammo';

                  if (useMap.isEmpty) {
                    return _HintCard(
                      icon: Icons.my_location_outlined,
                      title: 'No Working DOPE saved',
                      message:
                          'No Working DOPE saved for this firearm/ammo.',
                    );
                  }
                  ...
                  child: _seg == 0
                      ? _EquipmentList(
                          ...
                          items: rifles.map((r) {
                            ...
                            final verifiedCount = widget
                                .state
                                .ballisticDopeRecords
                                .where((e) => e.rifleId == r.id && e.isVerified)
                                .length;
                            final maintenance = widget.state
                                .maintenanceSnapshotForRifle(r.id);
                            ...
                            return Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 6),
                                  Text(
                                    'Caliber ${r.caliber} - ${r.manualRoundCount} rounds',
                                  ),
                                  const SizedBox(height: 8),
                                  Wrap(
                                    spacing: 6,
                                    ...
                                    children: [
                                      ColdBoreStatusPill(
                                        label:
                                            'Cold bore avg ${coldBoreAvg == null ? 'N/A' : '${coldBoreAvg.toStringAsFixed(2)} MOA'}',
                                      ),
                                      ColdBoreStatusPill(
                                    ...
                                    children: [
                                      Expanded(
                                        child: OutlinedButton.icon(
                                          ...
                                          label: const Text('Service Log'),
                                        ),
                                      ),
                                      const SizedBox(width: 8),
                                      Expanded(
                                        child: FilledButton.tonalIcon(
                                          onPressed: () => _editRifle(r),
                                          icon: const Icon(Icons.edit_outlined),
                                          label: const Text('Edit Rifle'),
                                        ),
                                      ),
                                    ],
                                  ),
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        children: [
                                          Expanded(
                                            child: FilledButton.tonalIcon(
                                              onPressed: () => _editAmmo(a),
                                              icon: const Icon(
                                                Icons.edit_outlined,
                                              ),
                                              label: const Text('Edit Ammo'),
                                            ),
                                          ),
                                          const SizedBox(width: 8),
                                          Expanded(
                                            child: OutlinedButton.icon(
                                              onPressed: () => _deleteAmmo(a),
                                              icon: const Icon(
          ...
          body: ListView(
            ...
            children: [
              const ColdBoreSectionHeader(
                title: 'Export',
                subtitle:
                    'Create polished reports and use presets for repeat workflows.',
              ),
              const SizedBox(height: 10),
              ColdBoreCard(
                padding: const EdgeInsets.all(12),
          ...
          child: Column(
            ...
            children: [
              ...
              if (actionLabel != null && onAction != null) ...[
                const SizedBox(height: 16),
                ColdBorePrimaryButton(label: actionLabel!, onPressed: onAction),
              ],
            ],
          ),
        ),
...
class _NewSessionDialog extends StatefulWidget {
  ...
  const _NewSessionDialog({this.existingFolders = const <String>[]});
              ...
              : ListView(
                  ...
                  children: [
                    const ColdBoreSectionHeader(
                      ...
                      subtitle:
                          'Track rifle health, reminder status, and service activity.',
                    ),
                    const SizedBox(height: 10),
                    ColdBoreCard(
                      padding: const EdgeInsets.all(12),
                  ...
                  (service) => ColdBoreCard(
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Icon(_maintenanceTaskIcon(service.taskType)),
                          const SizedBox(width: 10),
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                ...
                                const SizedBox(height: 4),
                                Text(
                                  '${_fmtDate(service.date)} - ${service.roundsAtService} total rds',
                                ),
...
class _RifleMaintenanceRulesScreenState
  ...
  Widget build(BuildContext context) {
    ...
    if (rifle == null) {
      return const ColdBoreScaffold(
        body: Center(child: Text('Rifle not found.')),
      );
    }
    final rifleModelLabel =
        '${(rifle.manufacturer ?? '').trim()} ${(rifle.model ?? '').trim()}'
      ...
      body: ListView(
        ...
        children: [
          const ColdBoreSectionHeader(
            ...
            subtitle:
                'Create full JSON backups or restore imported session and app data files.',
          ),
          const SizedBox(height: 10),
          ColdBoreCard(
            padding: const EdgeInsets.all(12),
            child: Wrap(
              ...
              children: [
                ColdBoreMetricTile(
                  label: 'Sessions',
                  value: '${state.allSessions.length}',
                  icon: Icons.event_note_outlined,
                ),
                ColdBoreMetricTile(
                  label: 'Rifles',
                  value: '${state.rifles.length}',
                  icon: Icons.sports_martial_arts_outlined,
                ),
                ColdBoreMetricTile(
                  label: 'Ammo',
                  value: '${state.ammoLots.length}',
                  icon: Icons.inventory_2_outlined,
                ),
              ],
            ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 10),
                  FilledButton.tonalIcon(
                    onPressed: () => _exportBackupFile(context),
          ...
          ColdBoreCard(
            child: ListTile(
              leading: const Icon(Icons.info_outline),
              title: const Text('Backup safety note'),
              subtitle: const Text(
                'Restoring will replace local data on this device. Keep recent backup files in a secure location.',
              ),
            ),
          ),
        ],
